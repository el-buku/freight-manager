...
import {Row} from '../components/dispatchers/DispatcherRow'
...
import AddDriverForm from '../components/dispatchers/AddDispatcherForm'
...
function DriversTable({drivers, classes}) {
    ...
    const deleteCallback = (name) => {
        firebase.firestore().collection('dispatchers').doc(name).delete().then(()=>{
...
export default function DispatchersPage({classes}) {
    const [add, setAdd] = useState(false)
    const [drivers, setDrivers] = useState([]);
    const getDrivers = ()=>{
        firebase.firestore().collection('dispatchers').get().then(
            (querySnapshot)=>{
                var customersCol=[]
    ...
    return (
        <React.Fragment>
            {formStatus!=''?
                <Alert color={formStatus=='error'?"error":"success"}onClose={() => {setStatus('')}} style={{marginBottom: '0.5em'}}>{formStatus=='success'?"Dispatcher added!":"Error"}</Alert>
                :null
            }
