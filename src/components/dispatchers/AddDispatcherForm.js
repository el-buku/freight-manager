...
export default function DispatcherForm(props) {
    ...
    const validateForm = () => {
        ...
        if (email!=''&&name!=''){
            const customer = {name, email, phoneNo, history:[]}
            fetch('/api/dispatcher_added', {method: 'POST', body: JSON.stringify(customer)}).then(
                (r)=>{
                    console.log(r)
                }
            )
            firebase.firestore().collection('dispatchers').doc(name).set(customer).then(()=>callback('success', customer)).catch(()=>callback('error'))
            setErr({})
        }else{
            var n,e
            if(name=='')n=true
