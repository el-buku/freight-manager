...
export default function CustomerForm(props) {
    ...
    const validateForm = () => {
        ...
        if (email!=''&&name!=''){
            const customer = {name, email, address, city, state, zip, phoneNo, contactPhoneNo, contactEmail, history:[]}
            firebase.firestore().collection('customers').doc(name).set(customer).then(()=>callback('success', customer)).catch(()=>callback('error'))
            setErr({})
        }else{
            var n,e
            if(name=='')n=true
    ...
    return (
        <div className={classes.root} style={query?{flexDirection:"column"}:null}>
            ...
            <TextField required label={"Phone"} variant={"outlined"} value={phoneNo} onChange={(event)=> {
            ...
            <TextField label={"Contact Phone"} variant={"outlined"} variant={"outlined"} value={contactPhoneNo} onChange={(event)=> {
