...
export default function FullWidthTabs({row, editCallback}) {
    ...
    const handleSave = () => {
        firebase.firestore().collection('brokers').doc(fnstateObj.name).set(fnstateObj).then(
            ()=>{
                setEdit(false);
                setAlert(true)
                firebase.firestore().collection('orders').map(
                    snapShot=>{
                        var data = snapShot.data()
                        if (data.broker.name==fnstateObj.name){
                            firebase.firestore().collection('orders').doc(data.id).set({...data, broker:fnstateObj})
                        }
                    }
                )
...
function Details({row, edit, callback}) {
    ...
    const sortArr = (arr) => {
        ...
        var ret = []
        if(arr.includes('phone')) ret  = ['name','address','city','state','zip','phone']
        else if(arr.includes('phoneNo'))ret  = ['name','address','city','state','zip','phoneNo']
        arr.map(key=> {
            if (!ret.includes(key)) ret.push(key)
        })
        return ret
    }
    const getMap = (stateObj) => sortArr(Object.keys(stateObj)).map(key => {
    ...
    return (
        <List dense style={{width: 300, display: 'flex', flexDirection: 'column', height: 300}}>
            {getMap(state)}
        </List>
    )
}
